...
class CVAE(Model):
    ...
    def build_encoder(self) -> None:
        ...
        self.encoder.add(Conv2D(filters=32, kernel_size=3, strides=2, padding="same"))
        self.encoder.add(Activation("relu"))
        self.encoder.add(Conv2D(filters=64, kernel_size=3, strides=2, padding="same"))
        self.encoder.add(Activation("relu"))
    ...
    def build_decoder(self) -> None:
        ...
        self.decoder.add(Conv2DTranspose(filters=64, kernel_size=3, strides=2, padding="same"))
        self.decoder.add(Activation("relu"))
        self.decoder.add(Conv2DTranspose(filters=32, kernel_size=3, strides=2, padding="same"))
        self.decoder.add(Activation("relu"))
        self.decoder.add(Conv2DTranspose(filters=1, kernel_size=3, strides=1, padding="same"))
...
def generate_and_save_images(model: CVAE, epoch: int, test_sample: tf.Tensor) -> None:
...
def main() -> None:
    ...
    for epoch in range(1, epochs + 1):
        ...
        elbo = -loss.result()
        print(
            f"Epoch: {epoch}, Test set ELBO: {elbo}, time elapse for current epoch: {end_time - start_time}"
        )
        if epoch % 10 == 0:
